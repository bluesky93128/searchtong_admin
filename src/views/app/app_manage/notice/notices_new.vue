        ...
        <b-card class="mb-4" title="공지사항 등록">
          <b-form>
            ...
            <div class="d-flex justify-content-center align-items-center">
              <b-button variant="primary" @click="onRegister()">등록</b-button>
            </div>
          </b-form>
        </b-card>
      </b-colxx>
...
export default {
  ...
  methods: {
    ...
    onRegister() {
      ...
      fetch(apiUrl + "/notice", requestOptions)
        .then(response => response.json())
        .then(result => {
          console.log(result)
          if(result.status) {
            this.addNotification("success filled", "공지사항 등록", "공지사항이 성공적으로 등록되었습니다.");
            this.$router.go(-1);
          }
        })
